...
class BonLivraisonPDF {
  ...
  static Widget buildHeader(BonLivraisonModel bonLivraisonModel, UserModel user) =>
      Column(
  ...
  static Widget buildCustomerAddress(BonLivraisonModel bonLivraisonModel, UserModel user) =>
      Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
  ...
  static Widget buildInvoice(BonLivraisonModel bonLivraisonModel, monnaie) {
    ...
    return pw.Column(
      ...
      children: [
        buildSimpleText1(title: 'Produit', value: bonLivraisonModel.idProduct),
        // SizedBox(height: 2 * PdfPageFormat.mm),
        Divider(),
        buildSimpleText1(title: 'Quantité Entrée', value: '${NumberFormat.decimalPattern('fr').format(double.parse(bonLivraisonModel.quantityAchat))} ${bonLivraisonModel.unite}'),
        // SizedBox(height: 2 * PdfPageFormat.mm),
        // Divider(),
        ...
        buildSimpleText1(title: 'TVA', value: '${bonLivraisonModel.tva} %'),
          // SizedBox(height: 2 * PdfPageFormat.mm),
        Divider(),
        buildSimpleText1(title: 'Prix de vente unitaire', value: '${NumberFormat.decimalPattern('fr').format(double.parse(bonLivraisonModel.prixVenteUnit))} $monnaie'),
        // SizedBox(height: 2 * PdfPageFormat.mm),
        
        if (double.parse(bonLivraisonModel.remise) >= 1)
        Divider(),
        if (double.parse(bonLivraisonModel.remise) >= 1)
        buildSimpleText1(title: 'Prix de Remise', value: '${bonLivraisonModel.remise} $monnaie'),
        // SizedBox(height: 2 * PdfPageFormat.mm),
        if (double.parse(bonLivraisonModel.qtyRemise) >= 1)
        Divider(),
        if (double.parse(bonLivraisonModel.qtyRemise) >= 1)
        buildSimpleText1(title: 'Qtés pour la remise', value: '${bonLivraisonModel.qtyRemise} ${bonLivraisonModel.unite}'),
        // SizedBox(height: 2 * PdfPageFormat.mm),
        // Divider(),
